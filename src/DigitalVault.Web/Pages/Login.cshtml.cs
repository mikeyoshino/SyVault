        ...
        try
        {
            ...
            if (result?.Success == true && result.Data != null)
            {
                ...
                var claims = new List<Claim>
                {
                    ...
                    // Store encryption metadata in claims for client-side decryption
                    new Claim("encryptedMasterKey", user.EncryptedMasterKey ?? ""),
                    new Claim("keyDerivationSalt", Convert.ToBase64String(user.KeyDerivationSalt ?? Array.Empty<byte>())),
                    new Claim("keyDerivationIterations", user.KeyDerivationIterations.ToString())
                };

                var identity = new ClaimsIdentity(claims, CookieAuthenticationDefaults.AuthenticationScheme);
                var authProperties = new AuthenticationProperties
                {
                    IsPersistent = true, // Always persist cookie (survives browser restart)
                    ExpiresUtc = DateTimeOffset.UtcNow.AddDays(30), // Cookie lasts 30 days
                    AllowRefresh = true
                };
                ...
                // Redirect to Blazor WASM app (root)
                return Redirect("/");
            }
